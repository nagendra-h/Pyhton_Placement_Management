...
#############  time
    ...
    ######## Labels ++++++++++
        ...
        self.u_En1=Entry(self.root,textvariable=self.var_en1,font=("times new roman" ,15),bg="white",bd=4,state='disabled')
        self.u_En1.place(x=701,y=240,height=40,width=210)
        
        
        btn_exit=Button(self.root,text="Result",command=self.result,font=("times new roman",20,"bold"),bg="brown",bd=3,cursor="hand2",anchor="w").place(x=991,y=240,height=40,width=210)
    ...
    def clear(self):
        ...
        self.u_En1.config(state='disabled')
    ...
    def result(self):
        ...
        try:
            if self.var_stud_id.get() == "":
                ...
                cur.execute("select status,company from student where stud_id=? ",(self.var_stud_id.get(),))
                user=cur.fetchone()
                if user==None:
                    messagebox.showerror("Error", "Please enter valid data",parent=self.root)
                ss=user[0]
                s1=user[1]
                self.u_En.config(state='normal')
                self.u_En1.config(state='normal')
                self.var_en.set(ss)
                self.var_en1.set(s1)
